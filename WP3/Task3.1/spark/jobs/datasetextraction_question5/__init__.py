...
# ============================================================================ #
# MAIN FUNCTION -- name of the task is datasetextraction_question5  #
# ============================================================================ #
def analyze(ss, cfg):
    """
    Run job
    ...
    logger.info('Extracting dataset of citation counts with OA status for all papers published by all THE WUR universities in the countries of our choice.')
    ...
    papers_df = ss.table(db_name + '.papers').select(['paperid', 'year', 'citationcount']).drop_duplicates()  # this will be used to extract citation counts for papers.
    ...
    for country_name, univ_names in cfg['data']['all_THE_WUR_institutions_by_country'].items():
        ...
        for univ_name in univ_names:
            ...
            if country_papers_oa_citations_df is None:
                country_papers_oa_citations_df = univ_papers_oa_citations_df
            else:
                # https://datascience.stackexchange.com/a/27231
                country_papers_oa_citations_df = country_papers_oa_citations_df.union(univ_papers_oa_citations_df.select(country_papers_oa_citations_df.columns))

            logger.info("\nExtracted dataset for the university: " + univ_name)

        # save the data for the current country
        output_filename = join(cfg['hdfs']['onmerrit_dir'], "cc_oa_" + country_name + "_papers.csv" )
